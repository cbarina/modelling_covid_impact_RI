...
temp <- lapply(ts_forecast, as.data.frame)

forecasts <- bind_rows(temp) %>%
  mutate(iso_code = names(temp), mean = `Point Forecast`, lower_ci = `Lo 95`, upper_ci = `Hi 95`) %>%
  tibble() %>%
...
### Add ARIMA predictions to the data

Here we add the model prediction to the data, keeping only 2020, and calculate
deltas (coverage values in 2020: reported - expected) and associated confidence
intervals.
...
pop_vis <- ggplot(data = x_large, aes(x = year, y = coverage)) +
  theme_bw() +
  ...
  facet_wrap(~ country, nrow = 1, scales = "free_y", labeller = label_wrap_gen(25)) +
