...
fig_overall_results <- res %>% 
  ...
  geom_pointrange(alpha = .4, aes(xmin = lower_ci, xmax = upper_ci, color = confident_95)) +
...
lm_income_region <- lm(delta ~ income_group + region, data = res)

# ANOVA
anova(lm_income_region, lm_income)

# effect of income after accounting for region
lm_region_income <- lm(delta ~ region + income_group, data = res)

# ANOVA
anova(lm_region_income, lm_region)
...
summary_output <- res_missed %>%
  ...
  dplyr::select(country, iso_code, region, region, income_group,
                ...
                confident_95, surviving_infants,
  ...
  rename(Country = country,
         ...
         "ARIMA modelled expected coverage (max 95% CI)" = lower_ci,
         "ARIMA modelled expected coverage (min 95% CI)" = upper_ci,
         "ARIMA coverage 95% CI" = ci_width,
         "WUENIC reported 2020 coverage" = coverage,
         "Coverage delta" = delta,
         "Coverage delta (max 95% CI)" = lower_delta,
         "Coverage delta (min 95% CI)" = upper_delta,
